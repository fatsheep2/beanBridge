...
export class CmbProvider extends BaseProvider {
  ...
  protected async parseOrders(fileData: FileData): Promise<Order[]> {
    ...
    console.log('[Provider-CMB] 字段映射:', fieldMap);
    console.log('[Provider-CMB] 表头:', headers);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (order) {
          ...
          console.warn(`[Provider-CMB] 第 ${this.lineNum} 行解析失败，跳过`);
        }
      } catch (error) {
    ...
    console.log(`[Provider-CMB] 成功解析 ${orders.length} 条记录`);
    return orders;
  }

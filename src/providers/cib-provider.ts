...
export class CibProvider extends BaseProvider {
  ...
  protected async parseOrders(fileData: FileData): Promise<Order[]> {
    ...
    console.log('[Provider-CIB] 字段映射:', fieldMap);
    console.log('[Provider-CIB] 表头:', headers);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (order) {
          ...
          console.warn(`[Provider-CIB] 第 ${this.lineNum} 行解析失败，跳过`);
        }
      } catch (error) {
    ...
    console.log(`[Provider-CIB] 成功解析 ${orders.length} 条记录`);
    return orders;
  }
